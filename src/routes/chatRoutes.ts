...
interface TwilioStatusWebhook {
  ...
  ErrorMessage?: string;
  Timestamp?: string;
  [key: string]: any; // Para otros campos que puedan venir en el webhook
}
...
const fetchMediaWithRetry = async (
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    ...
    throw new Error(
      `Failed to fetch media from Twilio: ${res.status} ${res.statusText}`
    );
  }
...
router.post("/olfatea/receive-message", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        const audioName = `audio_${Date.now()}_${uuidv4().slice(
          0,
          8
        )}.${fileExtension}`;
        ...
        const metadata = {
          ...
          customMetadata: {
            ...
            transcriptionStatus: incomingMessage.includes(
        ...
        console.log(
          `Uploading audio: ${audioName} (${audioBuffer.length} bytes)`
        );

        // Función para subir el audio a Firebase Storage
        const uploadAudio = (): Promise<string> => {
              ...
              uploadTask.on(
                ...
                async () => {
                  try {
                    ...
                    console.log(
                      `Audio successfully uploaded to Firebase: ${audioUrl}`
                    );
                    ...
                    if (
                      ...
                      throw new Error(
                        "Invalid Firebase URL generated for audio"
                      );
                    }
                    ...
                    console.error(
                      "Error getting audio download URL:",
                      urlError
                    );
                    reject(
                      `Failed to get audio download URL: ${
      ...
      try {
        ...
        if (mediaContentType.includes("png")) {
          ...
          mediaContentType.includes("jpeg") ||
        ...
        const imageName = `image_${Date.now()}_${uuidv4().slice(
          0,
          8
        )}.${fileExtension}`;
        ...
        console.log(
          `Uploading image: ${imageName} (${imageBuffer.length} bytes)`
        );
              ...
              uploadTask.on(
                ...
                async () => {
                  try {
                    ...
                    console.log(
                      `Image successfully uploaded to Firebase: ${imageUrl}`
                    );
                    ...
                    if (
                      ...
                      throw new Error(
                        "Invalid Firebase URL generated for image"
                      );
                    }
      ...
      try {
        ...
        console.log(
          "Processing vCard contact with content type:",
          mediaContentType
        );
        ...
        console.log(
          `Uploading vCard: ${vCardName} (${vCardBuffer.length} bytes)`
        );

        // Función para subir el vCard a Firebase Storage
        const uploadVCard = (): Promise<string> => {
          return new Promise((resolve, reject) => {
            try {
              const uploadTask = uploadBytesResumable(
                storageRef,
              ...
              uploadTask.on(
                ...
                async () => {
                  try {
                    const vCardUrl = await getDownloadURL(
                      uploadTask.snapshot.ref
                    );
                    console.log(
                      `vCard successfully uploaded to Firebase: ${vCardUrl}`
                    );
                    ...
                    if (
                      ...
                      throw new Error(
                        "Invalid Firebase URL generated for vCard"
                      );
                    }
                    ...
                    console.error(
                      "Error getting vCard download URL:",
                      urlError
                    );
                    reject(
                      `Failed to get vCard download URL: ${
                        urlError instanceof Error
                          ? urlError.message
                          : "Unknown error"
                      }`
        ...
        if (contactPhone) contactInfo += ` - Tel: ${contactPhone}`;
        if (contactEmail) contactInfo += ` - Email: ${contactEmail}`;
        if (contactOrg) contactInfo += ` - ${contactOrg}`;
      ...
      try {
        ...
        if (
          ...
          if (contentLength && parseInt(contentLength) !== fileBuffer.length) {
            ...
            console.error(
              `Expected: ${contentLength} bytes, Got: ${fileBuffer.length} bytes`
            );
            throw new Error(
              `Excel file size mismatch: expected ${contentLength}, got ${fileBuffer.length}`
            );
          }
          ...
          if (fileBuffer.length < 512) {
            throw new Error(
              `Excel file too small (${fileBuffer.length} bytes), likely corrupted`
            );
          }
          ...
          console.log(
            "File magic bytes:",
            Array.from(magicBytes, (b) => "0x" + (b as number).toString(16).padStart(2, "0")).join(" ")
          );

          if (!isValidZip) {
            console.error("❌ Excel file does not start with ZIP magic bytes");
            throw new Error(
              "Excel file appears to be corrupted - invalid file signature"
            );
          }
        ...
        if (mediaContentType.includes("pdf")) {
          ...
          mediaContentType.includes("spreadsheetml.sheet") ||
          mediaContentType.includes("vnd.ms-excel")
        ) {
          // Archivos Excel (.xlsx, .xls)
          fileExtension = "xlsx";
        } else if (
          mediaContentType.includes("word") ||
          mediaContentType.includes("wordprocessingml.document")
        ) {
          // Archivos Word (.docx, .doc)
          fileExtension = "docx";
        } else if (
          mediaContentType.includes("presentationml.presentation") ||
          ...
          mediaContentType.includes("text/csv") ||
          ...
          mediaContentType.includes("application/x-rar") ||
        ...
        const fileName = `document_${timestamp}_${uuidv4().slice(
          0,
          8
        )}.${fileExtension}`;
        ...
        console.log(
          `File detected as: ${detectedFileType} with extension: ${fileExtension}`
        );
              ...
              uploadTask.on(
                ...
                async () => {
                  try {
                    ...
                    console.log(
                      `File successfully uploaded to Firebase: ${fileUrl}`
                    );
    ...
    const incomingMessageId = await savePetAppMessage(
      fromNumber,
      incomingMessage,
      "user",
      mediaUrlToSave || undefined,
      incomingMessageSid || undefined
    );
    ...
    if (chatOn === true) {
      ...
      console.log(
        "⚠️ WARNING: chatOn is null - No conversation found, defaulting to HUMAN attention"
      );
      res.writeHead(200, { "Content-Type": "text/xml" });
      res.end(twiml.toString());
      return;
    ...
    if (incomingImage) {
      ...
      agentOutput = await agentWithPhone.invoke(
        {
          messages: [message],
        },
        config
...
router.post("/olfatea/webhook/status", async (req: express.Request, res: express.Response) => {
    try {
      const {
        MessageSid,
        MessageStatus,
      ...
      if (shouldUpdate) {
        ...
        switch (mappedStatus) {
          case "sent":
            (updateData as any).sent_at = new Date().toISOString();
            break;
          case "delivered":
            (updateData as any).delivered_at = new Date().toISOString();
            break;
          case "read":
            (updateData as any).read_at = new Date().toISOString();
            break;
          case "failed":
        ...
        if (updateError) {
          ...
          console.log(
            `✅ Updated message ${MessageSid}: ${previousStatus} → ${mappedStatus}`
          );
        }
      }

      // SIEMPRE guardar en el historial (para trazabilidad completa)
      const historyData: MessageStatusLog = {
...
router.get("/olfatea/message-status/:sid", async (req, res) => {
  try {
    const { sid } = req.params;
